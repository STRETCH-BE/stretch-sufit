/**
 * Solutions — 6 product cards. Ukrainian.
 * File path: /components/sections/uk/solutions.tsx
 */
...
export function Solutions() {
  return (
    <section id="solutions" aria-label="Рішення" className="bg-bg py-24 md:py-32">
      <Container>
        <div className="mb-16 flex flex-col items-start justify-between gap-8 md:flex-row md:items-end md:gap-16">
          <FadeIn>
            ...
            <SectionTitle className="mt-5 text-white">
              Шість способів
              <br />
              <span className="it">створити нову стелю.</span>
            </SectionTitle>
          </FadeIn>
          <FadeIn delay={120}>
            <p className="max-w-md text-[16px] leading-[1.6] text-white/55">
              ПВХ безшовний до <strong className="font-semibold text-white">6,50 м</strong>{" "}
              — у фактурах мат, глянець, сатин, блискітки, текстура і більше. Поліестер для преміум-інтер'єрів. Одна система —{" "}
              <strong className="font-semibold text-white">Stretch</strong> — підлаштована під будь-яке приміщення.
            </p>
          </FadeIn>
        </div>
  ...
  return (
    ...
    >
      ...
      <TrackedCTA
        ...
        href={`/uk/rishennya/${product.slug}`}
      ...
      <TrackedCTA
        ...
        href={`/uk/rishennya/${product.slug}`}
        aria-label={`Подивитися: ${product.title}`}
