/**
 * Cities — Ukrainian.
 * File path: /components/sections/uk/cities.tsx
 */
...
// Only cities with a full page get their own URL; the rest link to the hub.
          ...
          <FadeIn delay={120}>
            <ul className="grid grid-cols-2 gap-1 md:grid-cols-3">
              {cities.map((city) => (
                <li key={city.slug}>
                  <Link
                    href={fullPages.has(city.slug) ? `/uk/natiazhni-steli/${city.slug}` : "/uk/natiazhni-steli"}
